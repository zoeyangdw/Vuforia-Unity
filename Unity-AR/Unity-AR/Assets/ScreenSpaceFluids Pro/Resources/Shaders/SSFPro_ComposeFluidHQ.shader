...
Shader "ScreenSpaceFluids/SSFPro_ComposeFluidHQ"
{
	Properties
	{
		...
		SubShader
	{
			...
			Pass
		{
			...
			//	texCoord2 = float2(i.coord.x, i.coord.y - _YFactor);
				...
				//float3 reflection = texCUBE(_Cube, reflectVec) * _Reflection;
				float3 reflection = pow(texCUBE(_Cube, reflectVec), _FresnelFalloff) * _Reflection;

		
				//REFRACTION
				float3 refraction = tex2D(_MainTex, i.uv + (normal.xy * thickness * _IndexOfRefraction)) * _Refraction;
